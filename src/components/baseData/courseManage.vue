...
export default {
  ...
  methods: {
    ...
    handleClickDelete (val) {
      // 点击删除按钮
      this.$axios.get('/course/deleteById?id=' + val.id)
        .then(() => {
          this.$emit('reload')
        })
    },
    handleClickConfirm () {
      // 点击确认按钮
      this.$axios.post('/course/saveOrUpdate', this.form)
        .then(() => {
          // 重置页面状态
          this.form = {}
    ...
    getCourseInfo () {
      // 加载请求
      this.$axios.get('/course/findAllCourse')
        .then(this.getCourseInfoSucc)
    },
    getCourseInfoSucc (res) {
      // 加载成功
