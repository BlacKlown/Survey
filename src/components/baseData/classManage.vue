          ...
          <el-dialog title="课程信息" :visible.sync="dialogFormVisible">
            <el-form :model="form">
              ...
              <el-form-item label="课程周期">
                <el-input-number v-model="form.period" :min="1" :max="30"></el-input-number>
              </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleClickDelete (val) {
      // 点击删除按钮
      this.$axios.get('/clazz/deleteClazzById?id=' + val.id)
        .then(() => {
          this.$emit('reload')
        })
    },
    handleClickConfirm () {
      // 点击确认按钮
      this.$axios.post('/clazz/saveOrUpdateClazz', this.form)
        .then(() => {
          // 重置页面状态
          this.form = {}
    ...
    getClassInfo () {
      // 加载请求
      this.$axios.get('/clazz/findAll')
        .then(this.getClassInfoSucc)
    },
    getClassInfoSucc (res) {
      // 加载成功
